...
## Version history ##
### 2.3.0-SNAPSHOT
* TODO: Add test case for annotate being called before first Tracy.before() throwing java.util.EmptyStackException
* TODO: Implement Tracy.annotate(String intName, long intValue)
* TODO: In case where an exception is thown `Tracy.frameError(String errorString)` can be used to close Tracy stack frame with a `errorString` error.
* TODO: In case where an exception is thown `Tracy.outerError(String errorString)` can be used to close all Tracy frames with a a generic error and the outer frame with `errorString`.
